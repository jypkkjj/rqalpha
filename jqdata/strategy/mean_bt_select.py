    ...
    try:
        for stock in stocks:
            ...
            if false_count >= 20:  # 确认由下向上趋势，均值突破1-3日
                if ma50_tp[length_ma50_tp - 1] or (ma50_tp[length_ma50_tp - 1] and ma50_tp[length_ma50_tp - 2]) or (
                        ma50_tp[length_ma50_tp - 1] and ma50_tp[length_ma50_tp - 2] and ma50_tp[length_ma50_tp - 3]):

                    # print("--------------股票%s 6日与108日移动均值比较结果------------ " % stock)
                    # print(ema_result3[length_ema_result3-10:])
                    # 底部突破
                    if ema_result1[length_ema_result1 - 1] and not ema_result2[length_ema_result2 - 1] and not \
                    ema_result3[length_ema_result3 - 1]:
                        ema_true_count = 0
                        ...
                        if ema_true_count > 1 and ema_true_count < 6 and ma_true_count > 1 and ma_true_count < 6:  # 突破小于一定时间
                            ...
                            print("股票还未突破", stock, 'ema6-18:', ema_result1[length_ema_result1 - 14:], 'ma50_tp',
                                  ma50_tp[length_ma50_tp - 10:])
            ...
            if if_red_ok:
                ...
                if_up = False
                if (macd[length_macd - 1] > macd[length_macd - 2] and macd[length_macd - 2] > macd[length_macd - 3] and
                    macd[length_macd - 3] > macd[length_macd - 4]) \
                    ...
                    if_up = True
                if if_up and ema_result1[length_ema_result1 - 1] and not if_weak and (ema6_18_true_count > 6 or (
                        ema_result3[length_ema_result3 - 1] and ema6_108_true_count > 0 and ema6_108_true_count < 4)):
                    print('股票满足强势条件ema6>18:6,ema6>108:3', stock, ',ema6-18：', ema_result1[length_ema_result1 - 10:],
                          'ema6-108', ema_result3[length_ema_result3 - 6:])
                    stocks_selected_median.append(stock)
                else:
                    print('股票未满足强势条件ema6>18:6,ema6>108:3', stock, ',ema6-18：', ema_result1[length_ema_result1 - 10:],
                          'ema6-108', ema_result3[length_ema_result3 - 6:])
            else:
            ...
            if ema_result1[length_ema_result1 - 1] and ema6_18_true_count > 12 and ema18_108_true_count > 12:
                print('股票满足超级强势条件ema6>18:30/50,ema18>108:30/50：', stock, 'ema6>18：', ema6_18_true_count, 'ema18>108：',
                      ema18_108_true_count)
                stocks_selected_high.append(stock)
            else:
                print('股票未满足超级强势条件ema6>18:30/50,ema18>108:30/50：', stock, 'ema6>18：', ema6_18_true_count, 'ema18>108：',
                      ema18_108_true_count)
